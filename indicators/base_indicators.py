...
def macd(data, period_long=26, period_short=12, period_signal=9,
    ...
    data['macd_val'] = data['ema' + str(period_short)] - data[
        'ema' + str(period_long)]
    data['macd_signal_line'] = data['macd_val'].ewm(ignore_na=False,
                                                    min_periods=0,
                                                    com=period_signal,
                                                    adjust=True).mean()
...
def acc_dist(data, trend_periods=21, open_col='<OPEN>', high_col='<HIGH>',
    ...
    for index, row in data.iterrows():
        if row[high_col] != row[low_col]:
            ac = ((row[close_col] - row[low_col]) - (
                    row[high_col] - row[close_col])) / (
...
def on_balance_volume(data, trend_periods=21, close_col='Close',
                      vol_col='Volume'):
    """
...
def price_volume_trend(data, trend_periods=21, close_col='Close',
                       vol_col='Volume'):
    """
...
def average_true_range(data, trend_periods=14, open_col='<OPEN>',
    ...
    data['atr'] = data['true_range'].ewm(ignore_na=False, min_periods=0,
                                         com=trend_periods, adjust=True).mean()
    if drop_tr:
...
def chaikin_oscillator(data, periods_short=3, periods_long=10,
                       high_col='<HIGH>',
                       low_col='<LOW>', close_col='Close', vol_col='Volume'):
    ac = pd.Series([])
    val_last = 0
    ...
    ema_long = ac.ewm(ignore_na=False, min_periods=0, com=periods_long,
                      adjust=True).mean()
    ema_short = ac.ewm(ignore_na=False, min_periods=0, com=periods_short,
                       adjust=True).mean()
...
def typical_price(data, high_col='<HIGH>', low_col='<LOW>',
                  close_col='Close'):
    data['typical_price'] = (data[high_col] + data[low_col] + data[
        close_col]) / 3
...
def ease_of_movement(data, period=14, high_col='<HIGH>', low_col='<LOW>',
                     vol_col='Volume'):
    for index, row in data.iterrows():
        if index > 0:
            midpoint_move = (row[high_col] + row[low_col]) / 2 - (
                    data.at[index - 1, high_col] + data.at[
                index - 1, low_col]) / 2
        else:
            midpoint_move = 0
...
def mass_index(data, period=25, ema_period=9, high_col='<HIGH>',
               low_col='<LOW>'):
    high_low = data[high_col] - data[
        low_col] + 0.000001  # this is to avoid division by zero below
    ema = high_low.ewm(ignore_na=False, min_periods=0, com=ema_period,
                       adjust=True).mean()
    ema_ema = ema.ewm(ignore_na=False, min_periods=0, com=ema_period,
                      adjust=True).mean()
    div = ema / ema_ema
...
def directional_movement_index(data, periods=14, high_col='<HIGH>',
                               low_col='<LOW>'):
    remove_tr_col = False
    ...
    for i, row in data.iterrows():
        if i > 0:
            data.set_value(i, 'm_plus',
                           row[high_col] - data.at[i - 1, high_col])
    ...
    data['adx'] = data['dxi'].ewm(ignore_na=False, min_periods=0, com=periods,
                                  adjust=True).mean()
...
def money_flow_index(data, periods=14, vol_col='Volume'):
    ...
    for index, row in data.iterrows():
        ...
        if index >= periods:
            ...
            positive_sum = data['money_flow_positive'][
                           index - periods:index].sum()
            negative_sum = data['money_flow_negative'][
                           index - periods:index].sum()
    ...
    data = data.drop(['money_flow', 'money_ratio', 'money_flow_positive',
                      'money_flow_negative'], axis=1)
...
def rsi(data, periods=14, close_col='Close'):
    ...
    data['rsi'] = data['rsi_u'].ewm(ignore_na=False, min_periods=0, com=periods,
                                    adjust=True).mean() / (
                          data['rsi_u'].ewm(ignore_na=False, min_periods=0,
                                            com=periods,
                                            adjust=True).mean() + data[
                              'rsi_d'].ewm(ignore_na=False, min_periods=0,
...
def chaikin_volatility(data, ema_periods=10, change_periods=10,
                       high_col='<HIGH>', low_col='<LOW>', close_col='Close'):
    data['ch_vol_hl'] = data[high_col] - data[low_col]
    data['ch_vol_ema'] = data['ch_vol_hl'].ewm(ignore_na=False, min_periods=0,
                                               com=ema_periods,
                                               adjust=True).mean()
    ...
    for index, row in data.iterrows():
        if index >= change_periods:
            ...
            if prev_value == 0:
                # this is to avoid division by zero below
                prev_value = 0.0001
            data.set_value(index, 'chaikin_volatility',
...
def williams_r(data, periods=14, high_col='<HIGH>', low_col='<LOW>',
               close_col='Close'):
    data['williams_r'] = 0.

    for index, row in data.iterrows():
        if index > periods:
            data.set_value(index, 'williams_r', ((max(
                data[high_col][index - periods:index]) - row[close_col]) /
                                                 (max(data[high_col][
                                                      index - periods:index]) - min(
                                                     data[low_col][
                                                     index - periods:index]))))
...
def trix(data, periods=14, signal_periods=9, close_col='Close'):
    data['trix'] = data[close_col].ewm(ignore_na=False, min_periods=0,
                                       com=periods, adjust=True).mean()
    data['trix'] = data['trix'].ewm(ignore_na=False, min_periods=0, com=periods,
                                    adjust=True).mean()
    data['trix'] = data['trix'].ewm(ignore_na=False, min_periods=0, com=periods,
                                    adjust=True).mean()
    data['trix_signal'] = data['trix'].ewm(ignore_na=False, min_periods=0,
                                           com=signal_periods,
                                           adjust=True).mean()
...
def ultimate_oscillator(data, period_1=7, period_2=14, period_3=28,
    ...
    for index, row in data.iterrows():
        if index > 0:
            ...
            bp = row[close_col] - min(row[low_col],
                                      data.at[index - 1, close_col])
    ...
    data = data.drop(['uo_bp', 'uo_tr', 'uo_avg_1', 'uo_avg_2', 'uo_avg_3'],
                     axis=1)
